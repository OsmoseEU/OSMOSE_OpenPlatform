...
public class CreateEaOutputInputFlowDialog extends AbstractCreateEaFlowDialog {
     ...
     */
    public CreateEaOutputInputFlowDialog(Shell parentShell, EaProcess process,EaFlowNode source, EaFlowNode target, EAStarAppService service) {
		super(parentShell, process, source, target, service, "Create a Output/Input flow");

    }

	@Override
	protected EaFlow createFlow(EaProcess process, EaFlowNode source, EaFlowNode target) {
		return getAppService().createInputOutputFlow(process, source, target);
	}
