...
public class EaToOsmosisProcessBpmnWizardPageSelectFile extends WizardPage {
	/** page name */
	public static String PAGE_NAME = "wizard.select.from";
	/** area for select eastar file */
	 ...
	 * @param pageName
	 */
	public EaToOsmosisProcessBpmnWizardPageSelectFile(ISelection selection, EaToOmosisProcessBpmnWizardContent content) {
		super(PAGE_NAME);
		setTitle("Select an EaStar file");
		setDescription("Select an EaStar file and enter a new  name for create a BPMN file.");
	...
	public void updateFileName() {
		IFile file = getFile();
		if (null!= file && "eastar".equals(getFile().getFileExtension())) {
			
			try{
				content.osmoseService = new OsmoseService();
				content.osmoseService.buildContainer(UtilsFolder.findFile(fileSelect.getSelection()));
				content.osmioticProcess = content.osmoseService.getOsmioticProcess();
			}
			catch(Exception e){
				System.out.println("Debug: EaToOsmioticProcessBpmnWizardPageSelectFile! - updaeFileName");
			}
	...
	private void dialogChanged() {
		if (null == getFile()) {
			updateStatus("Eastar file must be selected");
			return;
		} else if (!"eastar".equals(getFile().getFileExtension())) {
			updateStatus("File selected must be Eastar file");
			return;
		} else if (null == getFileName()
				|| getFileName().length() == 0
				|| this.defaultName.equals(getFileName())) {
			updateStatus("Eastar file name must be specified");
			return;
