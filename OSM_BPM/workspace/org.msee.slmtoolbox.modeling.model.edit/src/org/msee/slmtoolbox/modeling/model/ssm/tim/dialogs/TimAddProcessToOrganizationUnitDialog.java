 ...
 *
 */
public class TimAddProcessToOrganizationUnitDialog extends AbstractTimElementDialog<TimOrganizationUnit, TimProcess> {
	 ...
	 * 
	 */
	public TimAddProcessToOrganizationUnitDialog(Shell parentShell, ILabelProvider labelProvider,
			TimOrganizationUnit elementBeCompleted) {
	...
	protected List<TimProcess> getListSource() {
		...
		if(null != getElementBeCompleted() && null != getElementBeCompleted().getOrganization()) {
			listSource.addAll(getElementBeCompleted().getOrganization().getModel().getProcesses());
		}
		return listSource;
	}
	...
	protected void addNewElementToElementTarget(TimProcess newElement) {
		newElement.setResponsibleOrganizationUnit(getElementBeCompleted());
